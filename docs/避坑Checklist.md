...
- [ ] **电脑连不上树莓派的 IPv4（`ssh 192.168.1.5` 超时），但 IPv6 能连、ARP 能解析、树莓派能 ping 通网关**
      根因：Clash Verge 的 **TUN 模式**会劫持局域网 IPv4（只用「系统代理」不会有这问题）。
      特征：`arp -a` 能看到树莓派 MAC、树莓派能通 `192.168.1.1`，但电脑这边 TCP 22 超时。
      解决：**关掉 Clash 的 TUN 模式**（`系统代理` 保持开启即可照常上网），或给
      `192.168.1.0/24`（乃至 `192.168.0.0/16`）加直连规则。关 TUN 后可能需要等几十秒才生效。
