...
- 阶段六（树莓派上位机）进行中：SSH 公钥免密已通；WiFi 已连（192.168.1.5，已关 WiFi 省电）；
  **Pi ↔ CH32 串口已打通**；**Web 控制台已上线**：`pi/server.py` + `lintlizard-web.service`，
  ...
  固件编译及 Pi 7 项测试、C 解析器主机测试通过；尚未烧录、尚未验证真实网页点云。
  当前用户确认雷达未接线，已告知 TX→PA3/J3-34、GND→J4-7、VCC→独立5V、M_CTR→3.3V。
  下一步等接线和供电完成，提醒覆盖后烧录，核对 COM4 [rad]/[scan] 与网页 /scan 数据。
...
* 待收尾：雷达重新接线、新固件烧录与网页真实点云验证；AAT-1、Pi UART、摄像头和摇杆已完成。
