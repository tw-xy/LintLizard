...
- 阶段六（树莓派上位机）进行中：系统已刷入；SSH 公钥免密已通；WiFi 已连（192.168.1.5）；
  **Pi ↔ CH32 串口已打通**（Pi 的 /dev/serial0 115200 → AAT-1 → CH32 USART3，发 {"x":50,"y":80}
  CH32 收到并解算，LINK-OK / frames 涨）；待办：摄像头 MJPEG + 点云网页 + 虚拟摇杆
