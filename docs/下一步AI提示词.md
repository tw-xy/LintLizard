...
- 当前树莓派：Raspberry Pi OS Lite 64-bit (Trixie, kernel 6.18.50+rpt-rpi-v8)；hostname lintlizard；
  用户 pi，SSH 公钥免密已通（本机私钥 C:\Users\hp\.ssh\lintlizard_ed25519）；
  WiFi 已连 miaomiaomao2.4G → 192.168.1.5（autoconnect）；Ethernet 备用 192.168.1.213；
  已实测拔网线后仅靠 wlan0 也能稳定 SSH
- ⚠ PC 侧：Clash Verge 的 **TUN 模式**会劫持局域网 IPv4（症状：`ssh 192.168.1.5` 超时、
  但 IPv6 通、ARP 通）；已关闭 TUN、保留系统代理。若以后重开 TUN，需给 `192.168.1.0/24` 加直连规则
...
【协议】ESP→CH32：115200 8N1，一行一帧 {"x":-100..100,"y":-100..100}\n
  Pi 网页指令额外带 "manual":1：纯手动，松手停止，S=-100 正常后退，不武装自动巡航。
  ESP/旧指令无 manual 字段仍使用旧巡航手势；避障和链路失效保护继续生效。
...
- 阶段六（树莓派上位机）进行中：SSH 公钥免密已通；WiFi 已连（192.168.1.5，已关 WiFi 省电）；
  **Pi ↔ CH32 串口已打通**；**Web 控制台已上线**：`pi/server.py` + `lintlizard-web.service`，
  浏览器 http://192.168.1.5:8080/ 有虚拟摇杆 + 摄像头 MJPEG（OV5647 实测 ~12fps）；
  点云转发代码已完成：CH32 每 8 个有效点发 1 行 R,angle_cdeg,dist_mm；非阻塞整行入队。
  Pi 接收服务已部署，按度保存最新点、0.5s 过期；/scan 与 /status.radar 提供在线状态/计数。
  固件编译及 Pi 7 项测试、C 解析器主机测试通过；2026-09-22 已烧录并校验成功。
  雷达已接回，真实网页点云与摄像头同时显示：约409点/秒，140–177个新鲜点，Pi解析错误0。
  双向串口测试50条回中指令通过：LINK-OK、err=0、rxovf=0，电调1500/1500µs、roam=IDLE。
  CH32转发启动初期drop=6，后续稳定不增。截图：docs/images/20260922_03_web_radar_live.png。
  2026-09-22 追加 WASD 键盘驾驶（已部署网页+烧录配套 manual 固件）：默认50%速度，
  可调20%~100%，支持组合键；空格/Esc/松键/失焦停车，摇杆保留，100ms持续发送。
  8项Python/PTY测试、JS真实控制脚本模拟测试、C手动/巡航回归测试通过；实车仅静态回中验证。
  最新状态：用户已断电，晚上再上电测试；配套固件在断电前已烧录校验成功，网页也已部署。
  下一步验证真实按键行驶（非零方向尚未实测），不要因对话中断误以为尚未烧录。
...
* 已完成：系统刷入、SSH 公钥免密登录、WiFi 连上英文 2.4G SSID、AAT-1 3.3V UART 及 GPIO14/15 确认。
* 已完成：雷达接线、新固件烧录与真实网页点云；AAT-1、Pi UART、摄像头和摇杆均已完成。
* 注意：路由器实际 2.4G SSID 是 `miaomiaomao2.4G`（末音节 `mao`），5G 是 `miaomiaomiao5G`；
  ESP8266 的 `secrets.h` 仍是旧中文 `妙妙喵-2.4G`，继续用 ESP 时需改成英文 SSID。
