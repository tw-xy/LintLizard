...
>
> 本仓库是它的**固件与文档**，会跟着功能一起长大。目前已经完成
> **遥控底盘、电调驱动、OLED 状态显示、扫地执行机构和激光雷达前向避障**；
> 后面继续加入自动回充、树莓派建图、编码器闭环等功能。
...
| 二、电调输出（TIM3 双路 50Hz / 1000–2000µs PWM） | ✅ 已实测；轮子转向已确认 |
...
| 六、树莓派上位机（Web 控制台） | ✅ WiFi + SSH + 串口 + MJPEG + 网页摇杆 + **真实雷达点云**；新固件已烧录，约 409 点/秒，网页实测显示，双向串口验证通过 |
| 后续 | 落地跑直线/原地转 / 避障阈值精调 / 树莓派建图 / 编码器闭环（架构已预留接口） |

## Web 控制台实测

虚拟摇杆、W/A/S/D 键盘、CSI 摄像头 MJPEG 与实时雷达点云同屏显示。
W 前进、S 后退、A 左转、D 右转，支持组合键；松开、空格、Esc 或切走窗口停车。
键盘速度默认 50%，可调 20%～100%；网页指令使用纯手动模式，松手保持停止。

![WASD 键盘控制界面](docs/images/20260922_05_web_wasd.png)

2026-09-22 用户提供的点云联调截图：

![LintLizard Web 控制台：摇杆、摄像头与实时雷达点云](docs/images/20260922_04_web_console_user.png)
